...
### Added — "Use mock data" (Settings → Connections)

- **One-click demo tenant** (`lib/demo-seed.ts`, `settings/connections/demo-data.tsx`,
  `loadDemoData` action). Admin-only, sitting right beneath "Run sync now", it
  fills the active brand with a coherent end-to-end dataset so every screen is
  testable without a live Meta account: a fortnight of published/scheduled/draft/
  pending-approval posts across the brand's pillars, a full analytics history, a
  ...
  report.
- **Works in any mode, never calls Meta**: account and post insights (metricsDaily,
  postMetrics + intent) are generated by the mock adapter directly and written in
  the same shapes the real sync produces — so it behaves identically whether
  `USE_MOCK_ADAPTERS` is on or off, spends no Graph quota, and doesn't depend on
  a connection existing.
- **Never overwrites a real connection**: mock FB/IG connections are seeded only
  when the brand has none; a brand with a live connection keeps it untouched.
- **Idempotent**: every seeded doc has a deterministic `demo_{brandId}_…` id, so
  ...
  use (DECISIONS #031). Shares the sync rate-limit bucket.
